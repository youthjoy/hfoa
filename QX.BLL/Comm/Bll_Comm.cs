...
namespace QX.BLL
{
    public class Bll_Comm
    {
        ...
        public static string[] CompanyRoot
        {
            get
            {
                List<WH_Warehouse> ww = new BLL.Bll_WH_Warehouse().GetListByCode(string.Format("and Warehouse_PCode='{0}'", root));
                string[] WH_Warray = new string[ww.Count];
                for (int i = 0; i < ww.Count; i++)
                {
                    WH_Warray[i] = ww[i].Warehouse_Code;
                }
                return WH_Warray;
            }
        }
