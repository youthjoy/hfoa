...
namespace QX.BLL
{
   ...
   /// </summary>
   [Serializable]
   public partial class Bll_WH_Material
   {

        private ADOWH_Material instance = new ADOWH_Material();
        ...
        {
            List<WH_Material> list = instance.GetAll();
            return list;
        }
        ...
        /// <returns>list</returns>
        public List<WH_Material> GetListByCode(string strCondition)
        {
        ...
        /// <param name='strCondition'>条件(AND Code='11')</param>
        /// <returns>model</returns>
        public WH_Material GetModel(int id)
        {
            WH_Material model = instance.GetByKey(id);          
            return model;
        }    
        ...
        /// <param name='model'>实体</param>
        /// <returns>bool</returns>
        public bool Update(WH_Material model)
        {            
            bool result = false;
            var e = new ModelExceptions();
        ...
        {
            bool result = false;
            List<WH_Material> list = instance.GetListByWhere(Condition);
            if (list.Count > 0)
            {
                WH_Material model = list[0];
                model.Stat = 1;
                int _rseult = instance.Update(model);
                if (_rseult > 0)
                {
