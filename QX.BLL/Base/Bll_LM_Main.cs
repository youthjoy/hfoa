...
namespace QX.BLL
{
   ...
   /// </summary>
   [Serializable]
   public partial class Bll_LM_Main
   {

        private ADOLM_Main instance = new ADOLM_Main();
        ...
        {
            List<LM_Main> list = instance.GetAll();
            return list;
        }
        ...
        /// <returns>list</returns>
        public List<LM_Main> GetListByCode(string strCondition)
        {
        ...
        /// <param name='strCondition'>条件(AND Code='11')</param>
        /// <returns>model</returns>
        public LM_Main GetModel(int id)
        {
            LM_Main model = instance.GetByKey(id);          
            return model;
        }    
        ...
        /// <param name='model'>实体</param>
        /// <returns>bool</returns>
        public bool Update(LM_Main model)
        {            
            bool result = false;
            var e = new ModelExceptions();
        ...
        {
            bool result = false;
            List<LM_Main> list = instance.GetListByWhere(Condition);
            if (list.Count > 0)
            {
                LM_Main model = list[0];
                model.Stat = 1;
                int _rseult = instance.Update(model);
                if (_rseult > 0)
                {
