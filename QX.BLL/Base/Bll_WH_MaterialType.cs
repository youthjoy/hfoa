...
namespace QX.BLL
{
   ...
   /// </summary>
   [Serializable]
   public partial class Bll_WH_MaterialType
   {

        private ADOWH_MaterialType instance = new ADOWH_MaterialType();
        ...
        {
            List<WH_MaterialType> list = instance.GetAll();
            return list;
        }
        ...
        /// <returns>list</returns>
        public List<WH_MaterialType> GetListByCode(string strCondition)
        {
        ...
        /// <param name='strCondition'>条件(AND Code='11')</param>
        /// <returns>model</returns>
        public WH_MaterialType GetModel(int id)
        {
            WH_MaterialType model = instance.GetByKey(id);          
            return model;
        }    
        ...
        /// <param name='model'>实体</param>
        /// <returns>bool</returns>
        public bool Update(WH_MaterialType model)
        {            
            bool result = false;
            var e = new ModelExceptions();
        ...
        {
            bool result = false;
            List<WH_MaterialType> list = instance.GetListByWhere(Condition);
            if (list.Count > 0)
            {
                WH_MaterialType model = list[0];
                model.Stat = 1;
                int _rseult = instance.Update(model);
                if (_rseult > 0)
                {
