...
namespace QX.BLL
{
   ...
   /// </summary>
   [Serializable]
   public partial class Bll_SD_Project
   {

        private ADOSD_Project instance = new ADOSD_Project();
        ...
        {
            List<SD_Project> list = instance.GetAll();
            return list;
        }
        ...
        /// <returns>list</returns>
        public List<SD_Project> GetListByCode(string strCondition)
        {
        ...
        /// <param name='strCondition'>条件(AND Code='11')</param>
        /// <returns>model</returns>
        public SD_Project GetModel(int id)
        {
            SD_Project model = instance.GetByKey(id);          
            return model;
        }    
        ...
        /// <param name='model'>实体</param>
        /// <returns>bool</returns>
        public bool Update(SD_Project model)
        {            
            bool result = false;
            var e = new ModelExceptions();
        ...
        {
            bool result = false;
            List<SD_Project> list = instance.GetListByWhere(Condition);
            if (list.Count > 0)
            {
                SD_Project model = list[0];
                model.Stat = 1;
                int _rseult = instance.Update(model);
                if (_rseult > 0)
                {
