...
namespace QX.BLL
{
   ...
   /// </summary>
   [Serializable]
   public partial class Bll_SD_Contract
   {

        private ADOSD_Contract instance = new ADOSD_Contract();
        ...
        /// <returns>list</returns>
        public List<SD_Contract> GetListByCode(string strCondition)
        {
        ...
        /// <param name='strCondition'>条件(AND Code='11')</param>
        /// <returns>model</returns>
        public SD_Contract GetModel(int id)
        {
            SD_Contract model = instance.GetByKey(id);          
            return model;
        }    
        ...
        {
            bool result = false;
            List<SD_Contract> list = instance.GetListByWhere(Condition);
            if (list.Count > 0)
            {
